 ...
 * It takes pairs (row key, fuzzy info) to match row keys against. Where fuzzy info is
 ...
 *
 * I.e. fuzzy info tells the the matching mask is "????_99_????_01", where at ? can be any value.
 *
 */
public class FuzzyRowFilter extends FilterBase {
  ...
  public ReturnCode filterKeyValue(KeyValue kv) {
    ...
    // the only not handled SatisfiesCode is NO_NEXT, i.e. we are done
    done = true;
  ...
  public KeyValue getNextKeyHint(KeyValue currentKV) {
    ...
    if (nextRowKey == null) {
      ...
      throw new IllegalArgumentException("No next row key that satisfies fuzzy exists when" +
  ...
  private static SatisfiesCode satisfies(byte[] row, int offset, int length,
    ...
    for (int i = 0; i < fuzzyKeyMeta.length && i < length; i++) {
      ...
      boolean bytePositionFixed = fuzzyKeyMeta[i] == 0;
      boolean fixedByteIncorrect = bytePositionFixed && fuzzyKeyBytes[i] != row[i + offset];
      if (fixedByteIncorrect) {
        // in this case there's a other row that satisfies fuzzy rule and bigger than this row
      ...
      // case there's a byte array bigger than this row and which satisfies the fuzzy rule
  ...
  /**
   * NOTE: returns null if next doesn't exist
   */
  private static byte[] getNextForFuzzyRule(byte[] row, int offset, int length,
                                            byte[] fuzzyKeyBytes, byte[] fuzzyKeyMeta) {
    // To find out the next "smallest" byte array that satisfies fuzzy rule and "greater" the given
